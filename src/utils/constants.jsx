export const NAVIGATION = (isActive) => {
  return [
    {
      title: "Dashboard",
      icon: (
        <img
          src="/media/icons/dashboard.svg"
          className={`w-5 ${isActive("/") ? "filter-green" : "filter-gray"}`}
        />
      ),
      path: "/",
    },
    {
      title: "Sales",
      icon: (
        <img
          src="/media/icons/sales.svg"
          className={`w-5 ${
            isActive("/sales") ? "filter-green" : "filter-gray"
          }`}
        />
      ),
      path: "/sales",
    },
    {
      title: "Purchase",
      icon: (
        <img
          src="/media/icons/purchase.svg"
          className={`w-5 ${
            isActive("/purchase") ? "filter-green" : "filter-gray"
          }`}
        />
      ),
      path: "/purchase",
    },
    {
      title: "Inventory",
      icon: (
        <img
          src="/media/icons/inventory.svg"
          className={`w-5 ${
            isActive("/inventory") ? "filter-green" : "filter-gray"
          }`}
        />
      ),
      path: "/inventory",
    },
    {
      title: "Financials",
      icon: (
        <svg
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 24 24"
          className="w-5 filter-gray"
        >
          <path
            fillRule="evenodd"
            d="M12 2.25c-5.385 0-9.75 4.365-9.75 9.75s4.365 9.75 9.75 9.75 9.75-4.365 9.75-9.75S17.385 2.25 12 2.25ZM9 7.5A.75.75 0 0 0 9 9h1.5c.98 0 1.813.626 2.122 1.5H9A.75.75 0 0 0 9 12h3.622a2.251 2.251 0 0 1-2.122 1.5H9a.75.75 0 0 0-.53 1.28l3 3a.75.75 0 1 0 1.06-1.06L10.8 14.988A3.752 3.752 0 0 0 14.175 12H15a.75.75 0 0 0 0-1.5h-.825A3.733 3.733 0 0 0 13.5 9H15a.75.75 0 0 0 0-1.5H9Z"
            clipRule="evenodd"
          />
        </svg>
      ),
      children: [
        {
          title: "Expenses",
          path: "/financials/expenses",
        },
        {
          title: "Payments & Receipts",
          path: "/financials/payments-receipts",
        },
        {
          title: "Cash & Bank",
          path: "/financials/cash-bank",
        },
        {
          title: "Receivables & Payables",
          path: "/financials/receivables-payables",
        },
        {
          title: "Loans & ODs",
          path: "/financials/loans-ods",
        },
        {
          title: "Reports",
          path: "/financials/reports",
        },
      ],
    },
    {
      title: "Compliance",
      icon: (
        <svg
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 -960 960 960"
          className="w-5 h-5 filter-gray"
        >
          <path d="M571-270q11-10 12-25t-10-26L462-438l1-3h10q54 0 89.5-33t43.5-77h17q10 0 16.5-7t6.5-17q0-10-6.5-16.5T623-598h-18q-3-15-10.5-28.5T576-653h47q10 0 16.5-7t6.5-17q0-10-6.5-16.5T623-700H343q-12 0-20.5 8.5T314-671q0 12 8.5 20t20.5 8h127q26 0 42.5 13t22.5 32H337q-10 0-16.5 7t-6.5 17q0 10 6.5 16.5T337-551h199q-6 20-23 34.5T467-502h-68q-13 0-22.5 6T362-479q-5 11-3 22.5t11 21.5l150 164q10 11 25 11t26-10ZM480-80q-83 0-156-31.5T197-197q-54-54-85.5-127T80-480q0-83 31.5-156T197-763q54-54 127-85.5T480-880q83 0 156 31.5T763-763q54 54 85.5 127T880-480q0 83-31.5 156T763-197q-54 54-127 85.5T480-80Zm0-80q134 0 227-93t93-227q0-134-93-227t-227-93q-134 0-227 93t-93 227q0 134 93 227t227 93Zm0-320Z" />
        </svg>
      ),
      children: [
        {
          title: "GST",
          path: "/compliance/gst",
        },
        {
          title: "E-Way Bill",
          path: "/compliance/e-way-bill",
        },
        {
          title: "E-Invoice",
          path: "/compliance/e-invoice",
        },
        {
          title: "Other Taxes",
          path: "/compliance/other-taxes",
        },
        {
          title: "Audit Trail",
          path: "/compliance/audit-trail",
        },
      ],
    },
    {
      title: "Ledgers",
      icon: (
        <img
          src="/media/icons/ledgers.svg"
          className={`w-5 ${
            isActive("/ledgers") ? "filter-green" : "filter-gray"
          }`}
        />
      ),
      path: "/ledgers",
    },
  ];
};
