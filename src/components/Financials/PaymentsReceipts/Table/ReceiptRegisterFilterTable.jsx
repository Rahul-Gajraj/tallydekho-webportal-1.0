...
const ReceiptRegisterFilterTable = ({ isLoading }) => {
  const {
  ...
  return (
    <div>
      ...
      <table className="mt-4 min-w-full table-auto text-left">
        <thead>
          <tr>
            {RECEIPTS_TABLE_HEAD.map(({ head, customeStyle, value }, index) => (
              <th
                key={head}
              ...
              >
                <div className="flex">
                  ...
                  >
                    {head}
                    {index < RECEIPTS_TABLE_HEAD.length - 2 && (
        ...
        {isLoading ? (
          ...
          <tbody>
            {sortedReceiptTableRows.length > 0 ? (
              sortedReceiptTableRows.map((row, index) => {
                const {
                  date,
                  receiptNo,
                  party,
...
export default ReceiptRegisterFilterTable;
